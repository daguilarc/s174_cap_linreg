"""
Section 174 Capitalization Analysis - TCJA Regime (OCF)
=======================================================
...
Key Features:
- Tests R&D to book-to-tax difference impact on next period OCF margin
...
def main():
    ...
    essential_cols = ['cap_intensity', 'rnd_to_btd', 'ocf_margin_next']
    ...
    if X_clean.empty or y_clean.empty:
        ...
        plt.tight_layout()
        robust_savefig("rnd_deferral_vs_ocf_margin_v1.png", dpi=300, bbox_inches='tight')
        plt.close()
    else:
        ...
        if not X_clean.empty and not y_clean.empty:
            ...
            plt.title(f"Next Period OCF Margin vs R&D to Book-Tax Difference{fallback_suffix}", fontsize=16)
            plt.xlabel("R&D to Book-Tax Difference", fontsize=12)
            plt.tight_layout(rect=[0, 0, 1, 0.95])
            robust_savefig("rnd_to_btd_vs_ocf_margin_v0.png", dpi=300, bbox_inches='tight')
            plt.close()
        else:
    ...
    output_filename = "capitalization_analysis_output_v1.csv"
    ...
    diagnostic_filename = "diagnostic_statistics_v1.csv"
    ...
    if chart_selection['should_generate_chart']:
        ...
        chart_title = f"Next Period OCF Margin vs R&D over Book-to-Tax Difference — Top Statistically Significant Outliers"
        
        # Generate chart filename
        chart_filename = f"rnd_to_btd_vs_ocf_margin_v0.png"
    ...
    sp500_full_df_rnd = df_clean[['rnd_to_btd', 'ocf_margin_next', 'ticker']].dropna() if 'rnd_to_btd' in df_clean.columns and 'ocf_margin_next' in df_clean.columns and 'ticker' in df_clean.columns else pd.DataFrame()
