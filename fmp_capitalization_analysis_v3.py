...
def main():
    ...
    formula = f'ocf_margin_next ~ cap_intensity + rnd_deferral_ratio + leverage + revenue_growth + current_inventory_ratio + ocf_margin_lagged + profitability + ppe_total_assets + rnd_intensity + acquisitions_intensity + goodwill_change_intensity + intangible_assets_change_intensity + total_dna + market_cap + earnings_quality + quarter_number + year + C(sector_baseline, Treatment(reference="{reference_sector}"))'
    ...
    if X_clean.empty or y_clean.empty:
        ...
        plt.tight_layout()
        robust_savefig("cap_intensity_vs_ocf_margin_v3.png", dpi=300, bbox_inches='tight')
        plt.close()
        plt.figure(figsize=(12, 8))
        plt.title("No data available for regression chart", fontsize=16)
        plt.text(0.5, 0.5, "No data available", fontsize=16, color='gray', ha='center', va='center', transform=plt.gca().transAxes)
    ...
    plot_regression_chart(
        ...
        market_df=None,  # Add market data if available
        market_tech_df=None  # Add market tech data if available
    )
