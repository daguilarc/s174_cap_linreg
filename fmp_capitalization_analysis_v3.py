...
def safe_to_dict(df, orient='records'):
    """Safely convert DataFrame to dictionary format."""
    try:
        return df.to_dict(orient=orient)
    except Exception as e:
        print(f"[ERROR] Failed to convert DataFrame to dict: {e}")
        return []

def fetch_all_us_companies(api_key):
    """
    Fetch all US public companies from Financial Modeling Prep API.
    
    Args:
        api_key (str): Financial Modeling Prep API key
    
    Returns:
        pd.DataFrame: DataFrame with ticker, sector, and sub_sector columns
    """
    ALL_US_CACHE_FILE = "all_us_companies_cache.json"
    ALL_US_URL = "https://financialmodelingprep.com/api/v3/stock/list?apikey={api_key}"
    
    cache_expired = is_cache_expired(ALL_US_CACHE_FILE)
    if cache_expired and os.path.exists(ALL_US_CACHE_FILE):
        try:
            os.remove(ALL_US_CACHE_FILE)
            print(f"[CACHE] Deleted expired cache: {ALL_US_CACHE_FILE}")
        except Exception as e:
            print(f"[CACHE ERROR] Could not delete expired cache {ALL_US_CACHE_FILE}: {e}")
    
    cache = load_cache(ALL_US_CACHE_FILE)
    if cache and not cache_expired:
        return pd.DataFrame(cache)
    
    # If no cache or cache is expired, fetch from API
    if api_key is None:
        print("Warning: No all US companies cache found and no API key provided. Cannot fetch all US companies.")
        return pd.DataFrame()
    
    url = ALL_US_URL.format(api_key=api_key)
    response = requests.get(url)
    if response.status_code == 200:
        companies = response.json()
        # Filter for US companies only (exclude ETFs, mutual funds, etc.)
        us_companies = [company for company in companies if company.get('exchange') in ['NASDAQ', 'NYSE', 'AMEX']]
        
        # Create DataFrame and handle missing columns gracefully
        df = pd.DataFrame(us_companies)
        
        # Check what columns are actually available
        available_columns = df.columns.tolist()
        print(f"[DEBUG] Available columns in API response: {available_columns}")
        
        # Map expected columns to available columns with fallbacks
        column_mapping = {
            'symbol': 'symbol' if 'symbol' in available_columns else 'ticker',
            'sector': 'sector' if 'sector' in available_columns else 'industry',
            'industry': 'industry' if 'industry' in available_columns else 'sector'
        }
        
        # Select only available columns
        selected_columns = []
        for expected_col in ['symbol', 'sector', 'industry']:
            if column_mapping[expected_col] in available_columns:
                selected_columns.append(column_mapping[expected_col])
            else:
                print(f"[WARNING] Column '{expected_col}' not found in API response")
        
        if len(selected_columns) >= 1:  # At least symbol/ticker should be available
            df = df[selected_columns]
            # Rename columns to expected format
            if 'symbol' in df.columns:
                df = df.rename(columns={'symbol': 'ticker'})
            elif 'ticker' in df.columns:
                df = df.rename(columns={'ticker': 'ticker'})  # No change needed
            ...
            # Rename industry to sub_sector for consistency
            ...
            cache = safe_to_dict(df, 'records')
            save_cache(cache, ALL_US_CACHE_FILE)
            print(f"[API] Successfully fetched {len(df)} US public companies")
            return df
        else:
            print("[ERROR] No valid columns found in API response")
            return pd.DataFrame()
    else:
        print(f"[API ERROR] Failed to fetch all US companies: HTTP {response.status_code}")
        # Ensure no expired cache is used
        if os.path.exists(ALL_US_CACHE_FILE):
            try:
                os.remove(ALL_US_CACHE_FILE)
                print(f"[CACHE] Deleted expired cache after failed API call: {ALL_US_CACHE_FILE}")
            except Exception as e:
                print(f"[CACHE ERROR] Could not delete expired cache {ALL_US_CACHE_FILE}: {e}")
        return pd.DataFrame()

def get_market_tech(df):
    """
    Extract tech companies from all US companies data.
    """
    if df.empty:
        return pd.DataFrame()
    
    # Filter for tech companies based on sector
    tech_df = df[df['sector'].str.lower().str.contains('tech', na=False)]
    return tech_df
...
def main():
    """
    Main function that runs the complete analysis pipeline with proper caching logic.
    
    Caching Logic:
    1. If no cached data exists, prompt user for API key, fetch, cache, run code on cached data
    2. If cached data exists but is past 30-day expiration, prompt user for API key, cache data, run code on cached data  
    3. If cached data is available and not expired, do not prompt user for API key, just run on cached data
    """
    ...
    # Define essential caches
    essential_caches = [
        'all_us_companies_cache.json',
    ...
    print(f"\n[CACHE SUMMARY] Valid: {len(valid_caches)}, Expired: {len(expired_caches)}, Missing: {len(missing_caches)}")
    
    # Determine if API key is needed
    need_api = len(missing_caches) > 0 or len(expired_caches) > 0
    API_KEY = None
    
    if need_api:
        print(f"\n[API REQUIRED] Missing or expired caches detected. API key needed to update caches.")
        API_KEY = get_api_key()
        
        # Fetch and cache all required data
        print("[CACHE UPDATE] Fetching and updating all required caches...")
        
        # 1. Fetch all US companies
        all_companies_df = fetch_all_us_companies(API_KEY)
        tickers = all_companies_df['ticker'].tolist() if all_companies_df is not None and not all_companies_df.empty else []
        
        # 2. Fetch financial statements
        statements_data = fetch_statements_batch(tickers, API_KEY, batch_size=50)
        
        # 3. Fetch quotes and ratios
        ...
        all_companies_df = fetch_all_us_companies(API_KEY)  # This will load from cache
        tickers = all_companies_df['ticker'].tolist() if all_companies_df is not None and not all_companies_df.empty else []
    ...
    features = ['cap_intensity', 'rnd_deferral_ratio', 'leverage', 'revenue_growth', 'current_inventory_ratio', 
                'profitability', 'ppe_total_assets', 'rnd_intensity', 
                'acquisitions_intensity', 'goodwill_change_intensity', 'intangible_assets_change_intensity', 'total_dna', 'log_market_cap', 'log_total_assets', 'earnings_quality', 'quarter_number']
    
    # Initialize time_series_metrics
    time_series_metrics = {}
    
    print(f"Using all US public companies: {len(tickers)} companies")
    
    # Ensure all_companies_df is a DataFrame
    if not isinstance(all_companies_df, pd.DataFrame):
        all_companies_df = pd.DataFrame(all_companies_df)
    
    # Get tech tickers
    tech_tickers = []
    if not all_companies_df.empty and 'sector' in all_companies_df.columns:
        tech_tickers = all_companies_df[all_companies_df['sector'].str.lower().str.contains('tech')]['ticker'].tolist()
    
    # Ensure data structures are properly initialized
    if not statements_data or not hasattr(statements_data, 'items'):
        statements_data = {}
    
    if not isinstance(quotes_data, pd.DataFrame):
        quotes_data = pd.DataFrame()
    
    if not isinstance(ratios_data, dict):
        ratios_data = {}
    
    # Create market cap mapping
    market_cap_map = {}
    if not quotes_data.empty and 'symbol' in quotes_data.columns and 'marketCap' in quotes_data.columns:
        for _, row in quotes_data.iterrows():
            ticker = row['symbol']
            market_cap = row.get('marketCap', np.nan)
            ...
            try:
                ...
                if current_idx is not None and current_idx >= 3:
                    ...
                    flow_vars_is = {
                        ...
                        'dna_4q': is_full.get('depreciationAndAmortization', pd.Series(dtype=float))
                    }
                    ...
                    change_vars = {
                        ...
                        'ppe_change_4q': bs_full.get('propertyPlantEquipmentNet', pd.Series(dtype=float))
                    }
                    ...
                    dna_4q = all_4q_vars['dna_4q']
                    assets_4q = all_4q_vars['assets_4q']
                    debt_4q = all_4q_vars['debt_4q']
                    ppe_change_4q = all_4q_vars['ppe_change_4q']
    ...
    # Add sector information from all US companies data
    ...
    if not isinstance(all_companies_df, pd.DataFrame):
        all_companies_df = pd.DataFrame(all_companies_df)
    df = df.merge(all_companies_df[['ticker', 'sector', 'sub_sector']], on='ticker', how='left')
    ...
    print("\nVerifying GICS sector labels from all US companies API:")
    if not all_companies_df.empty and 'sector' in all_companies_df.columns:
        unique_sectors = all_companies_df['sector'].unique()
        print(f"Unique sectors from all US companies API: {unique_sectors}")
        print("Expected GICS sectors: Technology, Healthcare, Financial Services, Consumer Cyclical, etc.")
    else:
        print("Warning: No sector information available from all US companies API")
    ...
    essential_cols = ['cap_intensity', 'rnd_deferral_ratio', 'ocf_margin_next']
    ...
    tech_tickers = []
    if isinstance(all_companies_df, pd.DataFrame) and not all_companies_df.empty and 'sector' in all_companies_df.columns:
        all_companies_df['sector'] = all_companies_df['sector'].astype(str)
        tech_mask = all_companies_df['sector'].str.lower().str.contains('tech')
        tech_tickers = all_companies_df[tech_mask]['ticker'].tolist()
    ...
    # Check what sectors are actually available in the data
    available_sectors = df_clean_final['sector_baseline'].unique()
    print(f"Available sectors in data: {available_sectors}")
    
    # Determine the reference level - use the most common sector or fall back to first available
    sector_counts = df_clean_final['sector_baseline'].value_counts()
    print(f"Sector distribution: {sector_counts.to_dict()}")
    
    # Try to use "Software - Applications" if it exists, otherwise use the most common sector
    if "Software - Applications" in available_sectors:
        reference_sector = "Software - Applications"
        print(f"Using 'Software - Applications' as reference sector")
    else:
        reference_sector = sector_counts.index[0]  # Use the most common sector
        print(f"Using '{reference_sector}' as reference sector (most common)")
    
    # Create formula for OLS with industry fixed effects and quarter controls
    formula = f'ocf_margin_next ~ cap_intensity + rnd_deferral_ratio + leverage + revenue_growth + current_inventory_ratio + profitability + ppe_total_assets + rnd_intensity + acquisitions_intensity + goodwill_change_intensity + intangible_assets_change_intensity + total_dna + market_cap + earnings_quality + quarter_number + year + C(sector_baseline, Treatment(reference="{reference_sector}"))'
    ...
    if len(fe_coefficients) > 0:
        print(f"Industry Fixed Effects (relative to {reference_sector} baseline):")
        for industry, coef in fe_coefficients.items():
            industry_name = industry.replace(f'C(sector_baseline, Treatment(reference="{reference_sector}"))[', '').replace(']', '')
            print(f"  {industry_name}: {coef:.6f}")
        print(f"  {reference_sector}: 0.000000 (baseline/reference category)")
    else:
    ...
    if X_clean.empty or y_clean.empty:
        ...
        plt.tight_layout()
        robust_savefig("rnd_deferral_vs_ocf_margin_v3.png", dpi=300, bbox_inches='tight')
        plt.close()
    else:
    ...
    output_filename = "capitalization_analysis_output_v3.csv"
    ...
    # Create market data for trend lines
    market_full_df_rnd = df_clean[['rnd_deferral_ratio', 'ocf_margin_next', 'ticker']].dropna() if 'rnd_deferral_ratio' in df_clean.columns and 'ocf_margin_next' in df_clean.columns and 'ticker' in df_clean.columns else pd.DataFrame()
    market_tech_full_df_rnd = market_full_df_rnd[market_full_df_rnd['ticker'].isin(tech_tickers)] if not market_full_df_rnd.empty and 'tech_tickers' in locals() else pd.DataFrame()
    print('[TRENDLINE DIAGNOSTIC] rnd_deferral market shape:', market_full_df_rnd.shape, 'columns:', list(market_full_df_rnd.columns))
    print('[TRENDLINE DIAGNOSTIC] rnd_deferral market tech shape:', market_tech_full_df_rnd.shape, 'columns:', list(market_tech_full_df_rnd.columns))
    ...
    plot_regression_chart(
        ...
        chart_filename='rnd_deferral_vs_ocf_margin_v3.png',
        ...
        market_df=market_full_df_rnd,
        market_tech_df=market_tech_full_df_rnd
    )
    print("[DIAGNOSTIC] rnd_deferral_vs_ocf_margin_v3.png exported.")
    ...
    print(f"Significant features: {len(significant_features)} out of {len(['rnd_deferral_ratio', 'leverage', 'revenue_growth', 'current_inventory_ratio', 'ocf_margin_lagged', 'profitability', 'ppe_total_assets', 'rnd_intensity', 'acquisitions_intensity', 'goodwill_change_intensity', 'intangible_assets_change_intensity', 'total_dna', 'log_market_cap', 'log_total_assets', 'earnings_quality', 'quarter_number'])}")
    print(f"High-risk companies: {len(high_risk_companies)}")
    
    if 'market_data' in locals() and 'market' in market_data:
        print(f"\nMarket Context:")
        print(f"Market: ${market_data['market']['price']:,.2f} ({market_data['market']['changePercent']:+.2f}%)")
    else:
        print("[MARKET CONTEXT WARNING] Market data not available in market_data.")
    ...
    market_companies = ensure_pd_type(df_companies.copy(), 'dataframe')  # All market companies
    # Ensure tech_tickers is a list and not an ndarray
    if not isinstance(tech_tickers, list):
        tech_tickers = list(tech_tickers)
    market_tech_companies = ensure_pd_type(df_companies[df_companies['ticker'].isin(tech_tickers)].copy(), 'dataframe')  # Tech companies only
    ...
    print("Cache status summary not available - cache_statuses variable not defined")
    print("====================================================\n")
    ...
    # ... existing code ...
    print("[DIAGNOSTIC] Generating robust regression chart for rnd_deferral...")
    # Prepare full market and market tech DataFrames for the chart
    market_full_df_rnd = df_clean[['rnd_deferral_ratio', 'ocf_margin_next', 'ticker']].dropna() if 'rnd_deferral_ratio' in df_clean.columns and 'ocf_margin_next' in df_clean.columns and 'ticker' in df_clean.columns else pd.DataFrame()
    market_tech_full_df_rnd = market_full_df_rnd[market_full_df_rnd['ticker'].isin(tech_tickers)] if not market_full_df_rnd.empty else pd.DataFrame()
    print('[TRENDLINE DIAGNOSTIC] rnd_deferral market shape:', market_full_df_rnd.shape, 'columns:', list(market_full_df_rnd.columns))
    print('[TRENDLINE DIAGNOSTIC] rnd_deferral market tech shape:', market_tech_full_df_rnd.shape, 'columns:', list(market_tech_full_df_rnd.columns))
...
def plot_regression_chart(df, x_col, y_col, chart_filename, title, subtitle, date_range_str, market_df=None, market_tech_df=None):
    print(f"[TRENDLINE DIAGNOSTIC] market_df: shape={getattr(market_df, 'shape', None)}, columns={getattr(market_df, 'columns', None)}")
    print(f"[TRENDLINE DIAGNOSTIC] market_tech_df: shape={getattr(market_tech_df, 'shape', None)}, columns={getattr(market_tech_df, 'columns', None)}")
    ...
    if market_df is not None and not market_df.empty and x_col in market_df.columns and y_col in market_df.columns:
        X_market = market_df[[x_col]].values
        y_market = market_df[y_col].values
        if len(X_market) > 1:
            x_min, x_max = X_market.min(), X_market.max()
            x_vals = np.linspace(x_min, x_max, 100)
            model_market = LinearRegression().fit(X_market, y_market)
            y_market_vals = model_market.predict(x_vals.reshape(-1, 1))
            h_market = ax.plot(x_vals, y_market_vals, color='blue', linestyle='--', linewidth=2, label='Market Trend', zorder=3)[0]
            trend_handles.append(h_market)
    if market_tech_df is not None and not market_tech_df.empty and x_col in market_tech_df.columns and y_col in market_tech_df.columns:
        X_tech = market_tech_df[[x_col]].values
        y_tech = market_tech_df[y_col].values
        if len(X_tech) > 1:
            ...
            h_tech = ax.plot(x_vals_tech, y_tech_vals, color='purple', linestyle=':', linewidth=2, label='Market Tech Trend', zorder=3)[0]
    ...
    if y_col == 'ocf_margin_next':
        ax.set_ylabel("Next Period Operating Cash Flow Margin (4Q)", fontsize=14)
    else:
    ...
    # S&P 500 trend line
    if market_df is not None and not market_df.empty and x_col in market_df.columns and y_col in market_df.columns:
        X_sp = market_df[[x_col]].values
        y_sp = market_df[y_col].values
        if len(X_sp) > 1:
            model_sp = LinearRegression().fit(X_sp, y_sp)
            y_sp_vals = model_sp.predict(x_trend.reshape(-1, 1))
            h_sp = ax.plot(x_trend, y_sp_vals, color='blue', linestyle='--', linewidth=2, label='Market Trend', zorder=3)[0]
            trend_handles.append(h_sp)
            # Market Tech trend line
    if market_tech_df is not None and not market_tech_df.empty and x_col in market_tech_df.columns and y_col in market_tech_df.columns:
        X_tech = market_tech_df[[x_col]].values
        y_tech = market_tech_df[y_col].values
        if len(X_tech) > 1:
            ...
            h_tech = ax.plot(x_trend, y_tech_vals, color='purple', linestyle=':', linewidth=2, label='Market Tech Trend', zorder=3)[0]
            trend_handles.append(h_tech)
