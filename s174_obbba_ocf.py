"""
Section 174 Capitalization Analysis - OBBBA Regime
==================================================

This script analyzes earnings risks from over-capitalization of R&D under the One Big Beautiful Bill Act of 2025 (OBBBA),
which restored pre-TCJA full expensing status quo for Section 174 R&D expenses, but ONLY for R&D activities recorded 
in the United States. All R&D on foreign territory remains under the TCJA amortization schedule.

Key Features:
- Tests R&D deferral ratio impact on next period OCF margin
...
def main():
    ...
    if X_clean.empty or y_clean.empty:
        ...
        plt.tight_layout()
        robust_savefig("s174_obbba_ocf_rnd_deferral_vs_ocf_margin.png", dpi=300, bbox_inches='tight')
        plt.close()
    else:
    ...
    output_filename = "s174_obbba_ocf_output.csv"
    ...
    if time_series_metrics:
        ...
        ts_filename = "s174_obbba_ocf_time_series.csv"
    ...
    diagnostic_filename = "s174_obbba_ocf_diagnostics.csv"
    ...
    if chart_selection['should_generate_chart']:
        ...
        chart_title = f"Next Period OCF Margin vs R&D Deferral Proxy — Top Statistically Significant Outliers"
        
        # Generate chart filename
        chart_filename = f"s174_obbba_ocf_rnd_deferral_vs_ocf_margin.png"
        ...
        if selected_var in df_plot.columns and 'ocf_margin_next' in df_plot.columns:
        ...
        sp500_full_df = df_clean[[selected_var, 'ocf_margin_next', 'ticker']].dropna() if selected_var in df_clean.columns and 'ocf_margin_next' in df_clean.columns and 'ticker' in df_clean.columns else pd.DataFrame()
        ...
        if not sp500_full_df.empty:
            ...
            print(f'[TRENDLINE DEBUG] S&P 500 data range: ocf_margin_next = {sp500_full_df["ocf_margin_next"].min():.2f} to {sp500_full_df["ocf_margin_next"].max():.2f}')
        else:
        ...
        if not sp500_tech_full_df.empty:
            ...
            print(f'[TRENDLINE DEBUG] S&P 500 Tech data range: ocf_margin_next = {sp500_tech_full_df["ocf_margin_next"].min():.2f} to {sp500_tech_full_df["ocf_margin_next"].max():.2f}')
        else:
...
def plot_regression_chart(df, x_col, y_col, chart_filename, title, subtitle, date_range_str, sp500_df=None, sp500_tech_df=None, variable_info=None):
    ...
    # Title, labels - Always use R&D Deferral Proxy for x-axis
    ...
    if legend is not None:
            ...
            # Use the legend's actual left edge for proper alignment
            # Apply compensation to align subtitles with the legend box boundary
            ...
            # Debug output for legend positioning
...
def determine_significant_variable(model_with_fe, features):
    """
    Determine whether to generate chart based on model significance.
    Always uses R&D Deferral Proxy as the independent variable.
    ...
    # Always use R&D Deferral Proxy as the independent variable
    selected_var = 'rnd_deferral_ratio'
    
    # Define variable info for R&D Deferral Proxy
    variable_info = {
        'rnd_deferral_ratio': {
            'display_name': 'R&D Deferral Proxy',
            'formula': '(4Q CapEx / (4Q CapEx + (4Q Operating Expenses - 4Q R&D Expenses)))',
            'title_suffix': 'R&D Deferral Proxy'
        }
    }
