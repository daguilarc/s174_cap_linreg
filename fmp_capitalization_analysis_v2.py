...
- Industry-specific benchmarks and market comparisons
...
# === All US Companies Analysis ===
# Note: This version focuses on all US public companies (not S&P500)
...
def safe_to_dict(df, orient='records'):
    ...
    Args:
        ...
        orient (str): Orientation for conversion ('records' by default)
...
# Note: S&P500 data fetching removed - this version focuses on all US public companies

def fetch_company_profiles_for_sectors(tickers, api_key, cache_file="profile_cache.json"):
    """
    Fetch company profiles to get sector information for a list of tickers.
    ...
    Returns:
        dict: Dictionary mapping ticker to profile data
    """
    cache_expired = is_cache_expired(cache_file)
    cache = load_cache(cache_file)
    
    # Filter out tickers that are already cached and not expired
    tickers_to_fetch = [ticker for ticker in tickers if ticker not in cache or cache_expired]
    
    if not tickers_to_fetch:
        print(f"[CACHE] All {len(tickers)} tickers found in cache")
        return cache
    
    print(f"[API] Fetching profiles for {len(tickers_to_fetch)} tickers...")
    
    for i, ticker in enumerate(tickers_to_fetch):
        if i % 50 == 0:  # Progress update every 50 tickers
            print(f"[API] Progress: {i}/{len(tickers_to_fetch)} tickers processed")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data and len(data) > 0:
                    ...
                    cache[ticker] = {'sector': 'Unknown', 'industry': 'Unknown'}
            else:
                cache[ticker] = {'sector': 'Unknown', 'industry': 'Unknown'}
        except Exception as e:
            print(f"[ERROR] Failed to fetch profile for {ticker}: {e}")
            cache[ticker] = {'sector': 'Unknown', 'industry': 'Unknown'}
    ...
    save_cache(cache, cache_file)
    print(f"[API] Successfully fetched profiles for {len(tickers_to_fetch)} tickers")
    return cache
...
def fetch_income_statement(ticker, api_key, cache_file="income_statement_cache.json"):
    ...
    if not api_key:
        # Don't print individual messages for each missing cache to reduce noise
...
def main():
    ...
    if need_api:
        ...
        # Handle case where all_companies_df is empty or missing ticker column
        if all_companies_df.empty or 'ticker' not in all_companies_df.columns:
            print("[ERROR] No all US companies data available. Cannot proceed without company data.")
            print("[SOLUTION] Please provide an API key or ensure all US companies cache exists.")
            return
        
        tickers = all_companies_df['ticker'].tolist()
        ...
        # Update sector information from profiles
        print("[CACHE UPDATE] Updating sector information from company profiles...")
        for ticker in all_companies_df['ticker']:
            if ticker in profile_cache:
                profile = profile_cache[ticker]
                ...
                mask = all_companies_df['ticker'] == ticker
                all_companies_df.loc[mask, 'sector'] = sector
                all_companies_df.loc[mask, 'sub_sector'] = industry
        ...
        # Update sector information from cached profiles
        print("[CACHE] Updating sector information from cached company profiles...")
        for ticker in all_companies_df['ticker']:
            if ticker in profile_cache:
                profile = profile_cache[ticker]
                sector = profile.get('sector', 'Unknown')
    ...
    # Ensure all_companies_df is a DataFrame before using pandas methods
    if not isinstance(all_companies_df, pd.DataFrame):
        all_companies_df = pd.DataFrame(all_companies_df)
    
    # Handle NaN values in sector column before filtering for tech companies
    if 'sector' in all_companies_df.columns:
        # Fill NaN values with 'Unknown' to avoid the error
        all_companies_df['sector'] = all_companies_df['sector'].fillna('Unknown')
        # Now safely filter for tech companies
        tech_tickers = all_companies_df[all_companies_df['sector'].str.lower().str.contains('tech')]['ticker'].tolist()
    else:
        # If sector column doesn't exist, create empty list
        tech_tickers = []
    """
    ...
    # Use all US public companies (not S&P500)
    print("Using all US public companies for analysis")
    
    # Note: all_companies_df and tickers will be set in the cache loading section above
    # This section will use the tickers that were already fetched from all_companies_df
    
    # Ensure tickers is properly defined
    if 'tickers' not in locals() or not tickers:
        print("[ERROR] No tickers available. Cannot proceed without company data.")
        print("[SOLUTION] Please provide an API key or ensure all US companies cache exists.")
        return
    ...
    # Fetch quotes data for market capitalization
    quotes_data = fetch_quotes_in_batches(tickers, API_KEY)
    if not isinstance(quotes_data, pd.DataFrame):
        quotes_data = pd.DataFrame()
    
    # Create a mapping of ticker to market cap for easy lookup
    market_cap_map = {}
    ...
    formula = 'ocf_margin_next ~ cap_intensity + rnd_deferral_ratio + leverage + revenue_growth + current_inventory_ratio + ocf_margin_lagged + profitability + ppe_total_assets + rnd_intensity + acquisitions_intensity + goodwill_change_intensity + intangible_assets_change_intensity + total_dna + earnings_quality + quarter_number + year + C(sector_baseline, Treatment(reference="Software - Applications"))'
    ...
    all_companies = ensure_pd_type(df_companies.copy(), 'dataframe')  # All US companies
    # Ensure tech_tickers is a list and not an ndarray
    if not isinstance(tech_tickers, list):
        tech_tickers = list(tech_tickers)
    tech_companies = ensure_pd_type(df_companies[df_companies['ticker'].isin(tech_tickers)].copy(), 'dataframe')  # Tech companies only
    ...
    # Prepare full market and tech DataFrames for the chart
